...
from ganomaly.lib.test import seperate_image
...
class Castingdataset(Dataset):
    ...
    def __init__(self,root:str , train:bool,transform = None) -> None:
        ...
        for target_split in self._split2:
            ...
            for jpeg_path in self.files[self.split]:
               # _img = io.imread(jpeg_path)
                sep_img_list = seperate_image(jpeg_path)
                for sep_img in sep_img_list:
                    self.data.append(sep_img)
        ...
        self.data = np.vstack(self.data).reshape(-1, isize, isize, 3)
