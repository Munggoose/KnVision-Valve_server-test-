...
class BaseModel():
    ...
    def train(self):
        ...
        for self.epoch in range(self.opt.iter, self.opt.niter):
            ...
            if (self.epoch > 100 and self.epoch % 10):
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(self.dataloader['test'], 0):
                ...
                if self.opt.save_test_images:
                    ...
                    # cv2.imshow(f'{time.time() - startTime}', np.transpose(generated_img, (1,2,0)))
                    # cv2.waitKey(0)
                    ...
                    for bts in range(self.opt.batchsize):
                        ab_RGB.append(np.sum(diff_img[bts]) * 100 / (3*128*128))
                        ab_RGB_total.append(np.sum(diff_img[bts]) * 100 / (3*128*128))
                    diff_img[diff_img > thres] = 1

                    # real_img_clone = np.zeros(shape=(self.opt.isize, self.opt.isize, self.opt.nc))
                    anomaly_img = np.zeros(shape=(self.opt.batchsize, self.opt.isize, self.opt.isize, self.opt.nc))

                    anomaly_img = (real_img - diff_img + 1) /2 *255
                    #aanomaly_img[0] = (real_img - diff_img) *255

                    anomaly_img = anomaly_img + diff_img
                    # for realCH in range(self.opt.batchsize):
                        # real_img[realCH] = cv2.normalize(real_img[realCH], real_img[realCH], 0, 255, cv2.NORM_MINMAX)
                    ...
                    # cv2.imshow(f'{ab_RGB[0]}', np.transpose(anomaly_img, (1,2,0)))
                    # cv2.waitKey(0)
                    #print(f'scoring Time: {time.time() - startTime}')########################################################################################

                    # addImg = np.hstack((np.transpose(real_img, (1,2,0)), np.transpose(generated_img, (1,2,0)) ,np.transpose(aanomaly_img[0], (1,2,0))))
                    houseParty = []
                    ...
                    cv2.imwrite(f'output\\ganomaly\\casting\\test\\images\\diff_{i+1}_.png', addImg)


                    triImg = cv2.imread(f'output\\ganomaly\\casting\\test\\images\\diff_{i+1}_.png',cv2.IMREAD_UNCHANGED)
                    ...
                    for bts in range(self.opt.batchsize):
                        ...
                        cv2.putText(
                            scorePanel[bts],
                            f'{ab_RGB[bts + batchNum*self.opt.batchsize]: .4f}',
                        ...
                        cv2.putText(
                            scorePanel[bts],
                            f'{self.gt_labels[bts + batchNum*self.opt.batchsize]}',
                            position, #position at which writing has to start
                            cv2.FONT_HERSHEY_SIMPLEX, #font family
                            0.7, #font size
                            (255 ,255, 255, 255), #font color
                    ...
                    newImg = np.vstack((triImg, nnewImg))
                    ...
                    # position = (3,15)
                    # cv2.putText(
                    #      triImg, #numpy array on whievaluatech text is written
                    #      f'{ab_RGB: .1f},  {self.an_scores[i]: .4f}', #text
                    #      position, #position at which writing has to start
                    #      cv2.FONT_HERSHEY_SIMPLEX, #font family
                    #      0.5, #font size
                    #      (255 ,255, 255, 255), #font color
                    #      1) #font stroke
                    # cv2.imwrite(f'output\\ganomaly\\casting\\test\\images\\diff_{i+1}_.png', triImg)
                    #                     # print(f'an_score: {self.an_scores[i]}')
                    #                     # cv2.imwrite(f'output\\ganomaly\\casting\\test\\images\\diff{i+1}_{ab_RGB}.png', np.transpose(aanomaly_img[0], (1, 2, 0)))
            ...
            # for idx in range(len(self.gt_labels)):
            #     print(f'\t[{self.gt_labels[idx]}]: {self.an_scores[idx]}')
            ...
            auc = evaluate(self.gt_labels, self.an_scores, ab_RGB, self.epoch4Test, metric=self.opt.metric)
